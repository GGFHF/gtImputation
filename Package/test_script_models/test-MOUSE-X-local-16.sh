...
DATASET_ID=MOUSE
PROCESS_ID=A
NONMD_VCF_FILE=reference.vcf
WMD_VCF_FILE=miss_raw_10per.vcf
...
NONMD_VCF_PATH=$DATA_DIR/$NONMD_VCF_FILE
WMD_VCF_PATH=$OUTPUT_DIR/$WMD_VCF_FILE
SAMPLES_PATH=$DATA_DIR/$SAMPLES_FILE
...
{
    echo "$SEP"
    echo 'Loading data of the VCF file without missing data in the database ...'
    /usr/bin/time \
        load-vcf-data.py \
            --db=$DB_PATH \
            --vcf=$NONMD_VCF_PATH \
